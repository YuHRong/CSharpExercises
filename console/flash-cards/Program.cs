...
while (true)
{
  ...
  while (true)
  {
    ...
    {
      Clear();
      WriteLine("Flash Cards was closed.\n");
      return;
    }
  }
  ...
  while (!returnToMainMenu)
  {
    ...
    WriteLine("  What is the NATO phonetic alphabet code word for...\n");
    WriteLine($"{array[index].Letter}\n");
    ...
    {
      WriteLine($"  Incorrect. :( {array[index].Letter} ->  {array[index].CodeWord})");
    }

    Write("  Press [enter] to continue or [escape] to return to main menu...");
    while (true)
    {
      ConsoleKey key = ReadKey(true).Key;
      if (key is ConsoleKey.Enter)
